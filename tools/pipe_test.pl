...
sub load_test_cases {
  ...
  my $testcase = sub {
    ...
    my ($options, $input, @answers) = omit_lengthes(@_);
    push @{$cases[$#cases]->{tests}}, {options => $options, input => $input, answers => [@answers]};
  };
...
sub omit_lengthes {
  6 + $_[5] * 2 == @_ or die "Test case format error: wrong number of arguments for testcase()";
  @_[0, 2, 4, map { 5 + 2 * $_ } 1..$_[5]];
}
